...
func _ready() -> void :
    ...
    if turret:
        view_distance = turret.attack_range
...
func physics_update(delta: float) -> void :
    ...
    if is_player_visible:
        ...
        if _rotate_timer <= 0.0:
            ...
            var max_delta = deg_to_rad(turret.idle_rotate_range)
            var new_yaw = current_yaw + randf_range( - max_delta, max_delta)
            _target_yaw = new_yaw
            _rotate_timer = turret.idle_rotate_interval

        var current_yaw = pivot.rotation.y
        var angle_diff = wrapf(_target_yaw - current_yaw, - PI, PI)
        var step = turret.idle_rotate_speed * delta
