...
class_name PhaseTwoRunningFurnaceColossusState
...
func physics_update(delta: float) -> void :
    ...
    if dist <= attack_range:
        if not is_attacking_emitted:
            ...
            if attack_type < 2:
                colossus.top_state_machine.CURRENT_STATE.get_node("AttackStateMachine").on_child_transition("SlamAttackState")
            else:
                colossus.top_state_machine.CURRENT_STATE.get_node("AttackStateMachine").on_child_transition("SummonAttackState")
