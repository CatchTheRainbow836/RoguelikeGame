...
class_name PhaseTwoSplashEdenRemnantAttackState
...
var idle_attack_state: PhaseTwoIdleEdenRemnantAttackState
...
func _ready() -> void :
	...
	if boss:
		splash_damage = boss.phase2_splash_damage
		splash_radius = boss.phase2_splash_radius
		splash_count = boss.phase2_splash_count
		area_radius = boss.phase2_splash_area_radius
		player = PLAYER
	idle_attack_state = get_parent().get_node("IdleAttackState") as PhaseTwoIdleEdenRemnantAttackState
...
func enter() -> void :
	...
	if is_active:
		if idle_attack_state:
			idle_attack_state.splash_cooldown = boss.phase2_attack_cooldown
...
func _spawn_damage_circle(pos: Vector3, warning_circle: MeshInstance3D) -> void :
	...
	var material = StandardMaterial3D.new()
	material.albedo_color = Color(1, 0, 0, 1.0)
	material.transparency = BaseMaterial3D.TRANSPARENCY_ALPHA
	circle.material_override = material
	owner.get_parent().add_child(circle)
	circle.global_position = pos + Vector3(0, 0.05, 0)
