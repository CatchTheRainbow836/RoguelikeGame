...
func _ready() -> void :
	super._ready()
	await owner.ready
	var crawler = owner as ScrapCrawler
	if crawler:
		attack_damage = crawler.attack_damage
		attack_range = crawler.attack_range
		attack_cooldown = crawler.attack_cooldown
...
func perform_melee_attack() -> void :
	...
	var forward = - owner.get_node("Pivot").global_transform.basis.z.normalized()
	hitbox.global_position = owner.get_node("Pivot").global_position + forward * 1.5
