...
class_name GearWardenControlCenter
...
func _decide_action(delta: float) -> void:
	if can_see_player():
		...
		if dist <= attack_range:
			# Already in melee range: stop and attack
			_transition_movement("IdleEnemyState")
			_transition_attack("AttackingAttackState")
		elif dist <= attack_range * 2.0:
			# Within double range: run and attack (no shield)
			_transition_movement("RunningEnemyState")
			_transition_attack("AttackingAttackState")
		else:
			# Further than double range: run and shield
			_transition_movement("RunningEnemyState")
			_transition_attack("ShieldingAttackState")
		return
